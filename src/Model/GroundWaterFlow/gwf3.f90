...
! ------------------------------------------------------------------------------
    ...
    if(this%inhfb > 0) call this%hfb%hfb_ar(this%ibound, this%xt3d, this%dis,  &
                                         this%npf%inonstdf, this%npf%iflowform)
    ...
    !
    ! -- Ensure GNC is not being used at the same connection as a
    !    non-standard flow formulation
    if ((this%ingnc > 0).and.(this%npf%inonstdf /= 0)) then
      do ignc = 1, this%gnc%nexg
        ...
        if (this%npf%iflowform(iis) /= 0) then
          call store_error('Error in model ' // trim(this%npf%name_model) //   &
            '. A ghost node correction is specified at a connection that' //   &
            ' uses a non-standard flow formulation.')   ! amp_note: say which connection? cellids? gnc number?
          call ustop()
        end if
      enddo
    end if
